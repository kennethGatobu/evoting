...
<body>
  ...
  <!--end modals here -->
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h3 class="page-header">KCAU SAKU ELECTIONS RESULTS</h3>
					 <?php
                    show_campus_popup('show_elections.php',$con);
					//----------------------------------------------------------
                ...
                {	
                    ...
                    if($query=odbc_exec($con,"SELECT election_period,election_type 
                  ...
                  <table style="font:12px Tahoma, Arial, sans-serif" class="table table-striped table-bordered table-hover" id="dataTables-example">
					<thead>
                    <tr>
                    <th colspan="9"><h3 class="text-info">'.$_SESSION['campus_name'].'</h3></th>
                    </tr>
					<tr>
					<th></th>
                    ...
                    {
						echo '<tr><td>'.$i++.'</td>
                        <td>'.$row['election_period'].' '.$row['election_type'].
						'</td><td>'.
						'<a href="preview_results.php?election_period='.$row['election_period'].' &results=all">Preview Results</a>'.
						'</td><td>'.
						'<a href="preview_turnout_patterns.php?election_period='.$row['election_period'].'">Hourly turnout</a>'.
						'</td><td>'.
						'<a href="?election_periods='.$row['election_period'].'">Voters Register</a>'.
						'</td><td>'.
						'<a href="file_download_results.php?election_period='.$row['election_period'].'">
						<i class="fa fa-download"></i> Results for all Positions</a>'.
						'</td><td>'.
						'<a href="show_positions.php?ballot='.$row['election_period'].'">
						<i class="fa fa-download"></i> Results per Position</a>'.
						'</td><td>'.
						'<a href="file_download_turnout.php?turn_out='.$row['election_period'].'">
						<i class="fa fa-download"></i> Voter Turnout analysis</a>'.
						'</td><td>'.
						'<a href="file_download_list_voter_turnout.php?election_period='.$row['election_period'].'"><i class="fa fa-download"></i> List of all voters who voted</a>'.
                    ...
                    echo '<p class="alert alert-danger">Please select campus</p>';
                }
                    ?>
